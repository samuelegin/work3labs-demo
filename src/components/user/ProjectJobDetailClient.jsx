...
import { fetchProjectJob, fetchSystemMatches, acceptMatch, rejectMatch, fetchJobApplicants, acceptApplicant, projectVerifyAndRelease, projectRaiseDispute, submitProjectRating } from '@/services/api'
  ...
  return (
    <div className="min-h-screen bg-paper" style={{ fontFamily: 'Outfit, sans-serif' }}>
      ...
      <div className="max-w-[900px] mx-auto px-5 sm:px-8 py-10 sm:py-14 space-y-5">
        ...
        {/* Deal chat lives in the widget */}
        {['active', 'submitted', 'completed', 'disputed'].includes(job.status) && (
          <div className="bg-white border border-black/[0.07] rounded-[14px] px-5 py-4 flex items-center gap-3" style={{ animation: 'up 0.5s 0.08s both' }}>
            <i className="bi bi-chat-dots-fill text-[#3B82F6] text-[16px] flex-shrink-0" />
            <div className="flex-1 min-w-0">
              <p className="font-sans text-[13.5px] font-medium text-ink">Deal chat is in the widget</p>
              <p className="text-[12px] font-light text-[#888] mt-0.5">Continue the conversation with the Jobber via the chat bubble.</p>
            </div>
            <button
              onClick={() => window.dispatchEvent(new Event('open-chat-widget'))}
              className="flex items-center gap-1.5 font-mono text-[9px] tracking-[0.08em] uppercase text-[#3B82F6] border border-[#3B82F6]/20 bg-[#3B82F6]/5 rounded-full px-3 py-1.5 hover:bg-[#3B82F6]/10 transition-colors bg-transparent cursor-pointer flex-shrink-0"
            >
              <i className="bi bi-arrow-right text-[10px]" />Open
            </button>
          </div>
        )}
